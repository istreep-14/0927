...
const OUT_CSV = '/workspace/samples/chess-com/sample_row_ians141_2025-09.csv';
const URL = 'https://api.chess.com/pub/player/ians141/games/2025/09';
const ME = 'ians141';

function fetch(url) {
  return new Promise((resolve, reject) => {
    https.get(url, { headers: { 'User-Agent': 'cursor-sample-builder/1.0' } }, (res) => {
...
function formatDateLocalFromDate(d) {
  if (!d) return '';
  return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`;
}

function formatTimeLocalFromDate(d) {
  if (!d) return '';
  return `${String(d.getHours()).padStart(2,'0')}:${String(d.getMinutes()).padStart(2,'0')}:${String(d.getSeconds()).padStart(2,'0')}`;
}

function parseUtcDateTime(utcDate, utcTime) {
  if (!utcDate || !utcTime) return null;
  // utcDate like YYYY.MM.DD, utcTime like HH:MM:SS
  const [Y,M,D] = utcDate.split('.').map(Number);
  const [h,m,s] = utcTime.split(':').map(Number);
  if (!Y || !M || !D) return null;
  const d = new Date(Date.UTC(Y, M - 1, D, h||0, m||0, s||0));
  return d;
}
...
function deriveFormat(rules, timeClass) {
  if (rules === 'chess') return timeClass || '';
  if (rules === 'chess960') return timeClass === 'daily' ? 'daily960' : 'live960';
  const other = ['bughouse','crazyhouse','kingofthehill','threecheck','oddschess'];
  if (other.includes(rules)) return rules;
  return '';
}

function parseTimeControl(tc) {
  const out = { base: '', inc: '', corr: '' };
  if (!tc) return out;
  if (tc.includes('/')) {
    const parts = tc.split('/');
    out.corr = parts[1] ? String(parseInt(parts[1], 10)) : '';
    return out;
  }
  if (tc.includes('+')) {
    const [b,i] = tc.split('+');
    out.base = String(parseInt(b, 10));
    out.inc = String(parseInt(i, 10));
    return out;
  }
  out.base = String(parseInt(tc, 10));
  out.inc = '0';
  return out;
}

function deriveWinner(whiteRes, blackRes) {
  if (whiteRes === 'win' || blackRes === 'lose') return 'white';
  if (blackRes === 'win' || whiteRes === 'lose') return 'black';
  return '';
}
...
async function main() {
  ...
  const game = (json.games && json.games[0]) || null;
  ...
  const drv_type = timeClass === 'daily' ? 'daily' : 'live';
  const drv_format = deriveFormat(rules, timeClass);
  const tcParsed = parseTimeControl(game.time_control || '');
  const endDate = game.end_time ? new Date(game.end_time * 1000) : null;
  let startDate = null;
  if (game.start_time) startDate = new Date(game.start_time * 1000);
  else if (ph.UTCDate && ph.UTCTime) startDate = parseUtcDateTime(ph.UTCDate, ph.UTCTime);

  const drv_end = endDate ? formatDateTimeLocal(game.end_time) : '';
  const drv_end_date = endDate ? formatDateLocalFromDate(endDate) : '';
  const drv_end_time = endDate ? formatTimeLocalFromDate(endDate) : '';
  const drv_start = startDate ? `${formatDateLocalFromDate(startDate)} ${formatTimeLocalFromDate(startDate)}` : '';
  const drv_start_date = startDate ? formatDateLocalFromDate(startDate) : '';
  const drv_start_time = startDate ? formatTimeLocalFromDate(startDate) : '';
  const drv_duration = (endDate && startDate) ? String(Math.max(0, Math.floor((endDate - startDate)/1000))) : '';
  ...
  const valueByField = {
    ...
    drv_type,
    drv_format,
    drv_base_time: tcParsed.base,
    drv_increment_time: tcParsed.inc,
    drv_correspondence_time: tcParsed.corr,
    'white.rating': white.rating != null ? String(white.rating) : '',
    'white.result': white.result || '',
    'white.@id': white['@id'] || '',
    'white.username': white.username || '',
    'white.uuid': white.uuid || '',
    'black.rating': black.rating != null ? String(black.rating) : '',
    'black.result': black.result || '',
    'black.@id': black['@id'] || '',
    'black.username': black.username || '',
    'black.uuid': black.uuid || '',
    eco: game.eco || '',
    ...
    pgn_white: ph.White || '',
    pgn_black: ph.Black || '',
    pgn_result: ph.Result || '',
    pgn_eco_code: ph.ECO || '',
    pgn_eco_url: ph.ECOUrl || '',
    pgn_time_control: ph.TimeControl || '',
    pgn_termination: ph.Termination || '',
    pgn_start_time: ph.StartTime || '',
    pgn_end_date: ph.EndDate || '',
    pgn_end_time: ph.EndTime || '',
    pgn_link: ph.Link || '',
    pgn_opening: ph.Opening || '',
    pgn_variation: ph.Variation || '',
    pgn_current_position: ph.CurrentPosition || '',
    pgn_timezone: ph.Timezone || '',
    pgn_utc_date: ph.UTCDate || '',
    pgn_utc_time: ph.UTCTime || '',
    pgn_white_elo: ph.WhiteElo || '',
    pgn_black_elo: ph.BlackElo || '',
    pgn_setup: ph.SetUp || '',
    pgn_fen: ph.FEN || '',
    pgn_moves: pgnMoves,
    drv_end,
  ...
  const row = fields.map(f => csvEscape(valueByField[f] !== undefined ? valueByField[f] : '')).join(',');
